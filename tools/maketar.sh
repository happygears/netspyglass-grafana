...
then
    ...
    then
        tar -cf netspyglass-datasource-v2.tar --transform=s/^dist/$datasource_id/g --exclude test  dist/*
        exit
    fi
    ...
    then
        ...
        #
        tar -cf netspyglass-datasource-v2.tar -s "!^dist!$datasource_id!" --exclude test  dist/*
        exit
    fi
fi
